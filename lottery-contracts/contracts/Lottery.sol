...
contract Lottery is Ownable {
    ...
    /// @notice Lottery round number
    uint256 public lotteryRound;
    /// @notice Mapping of prize available for withdraw for each account
    mapping(address => uint256) public prize;
    /// @notice Struct that defines a lottery round winner
    struct RoundWinner {
        uint256 round;
        address winner;
        uint256 reward;
    }
    /// @notice Array of round winners
    ...
    /// @notice Function which deploys the LotteryToken
    /// @param tokenName Name of the token used for payment
    /// @param tokenSymbol Symbol of the token used for payment
    function createLotteryToken(string memory tokenName, string memory tokenSymbol) public onlyOwner {
        require(paymentToken == LotteryToken(address(0)), 'PaymenToken already set');
    ...
    modifier whenPaymentTokenSet() {
        require(paymentToken != LotteryToken(address(0)), "PaymentToken not set");
        _;
    }


    /// @notice Opens the lottery for receiving bets
    function openBets(uint256 closingTime) external onlyOwner whenBetsClosed whenPaymentTokenSet {
        require(
    ...
    function purchaseTokens() external payable whenPaymentTokenSet {
    ...
    function prizeWithdraw(uint256 amount) external whenPaymentTokenSet {
    ...
    function ownerWithdraw(uint256 amount) external onlyOwner whenPaymentTokenSet {
    ...
    function returnTokens(uint256 amount) external whenPaymentTokenSet {
